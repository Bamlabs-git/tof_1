...
if cmake --build $workpath/build --config Release --target example -j 4 &&
    ...
    cmake --build $workpath/build --config Release --target dual_preview_depth -j 4; then
    echo "== Build success"
    echo "== Run $workpath/build/example/cpp/example"
    echo "== Run $workpath/build/example/cpp/dual_preview_depth (for dual camera)"
else
    ...
    if cmake --build $workpath/build --config Release --target example &&
        ...
        cmake --build $workpath/build --config Release --target dual_preview_depth; then
        echo "== Build success"
        echo "== Run $workpath/build/example/cpp/example"
        echo "== Run $workpath/build/example/cpp/dual_preview_depth (for dual camera)"
    else
