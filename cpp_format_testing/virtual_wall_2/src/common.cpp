...
bool CameraManager::init(const Config& config) {
    camera_id_ = config.camera_id;
    return initialize();
}

std::vector<cv::Mat> CameraManager::getLatestFrames() {
    std::vector<cv::Mat> frames;
    
    ArducamFrameBuffer* frame = requestFrame(200);
    if (frame) {
        // Get frame format
        FrameFormat format;
        ...
        if (depth_ptr) {
            ...
            // ARDUCAM TOF DEPTH VALUE ANALYSIS
            static bool first_frame = true;
            if (first_frame) {
                first_frame = false;
                
                // Analyze depth value characteristics
                double min_val, max_val;
                cv::minMaxLoc(depth_frame, &min_val, &max_val);
                
                // Sample some depth values for analysis
                std::vector<float> samples;
                for (int y = 0; y < format.height; y += 10) {
                    for (int x = 0; x < format.width; x += 10) {
                        float val = depth_frame.at<float>(y, x);
                ...
                std::cout << "\n🔍 ARDUCAM TOF DEPTH ANALYSIS:" << std::endl;
                std::cout << "  Frame size: " << format.width << "x" << format.height << std::endl;
                std::cout << "  Depth range: " << min_val << " - " << max_val << std::endl;
                std::cout << "  Valid samples: " << samples.size() << std::endl;
                ...
                if (!samples.empty()) {
                    ...
                    std::cout << "  Sample statistics: mean=" << mean << ", median=" << median << std::endl;
                    ...
                    // Detect potential unit issues
                    if (mean > 10000.0f) {
                        std::cout << "  ⚠️ WARNING: Values seem very large - might be in micrometers (μm) instead of mm" << std::endl;
                        std::cout << "  ⚠️ Consider dividing by 1000 to convert μm to mm" << std::endl;
                    } else if (mean < 100.0f) {
                        std::cout << "  ⚠️ WARNING: Values seem very small - might be in meters instead of mm" << std::endl;
                        std::cout << "  ⚠️ Consider multiplying by 1000 to convert m to mm" << std::endl;
                    } else if (mean >= 300.0f && mean <= 4000.0f) {
                        std::cout << "  ✅ Values appear to be in millimeters (mm) as expected" << std::endl;
                    }
                }
                std::cout << std::endl;
            }
            
            frames.push_back(depth_frame.clone());
        }
        ...
        if (confidence_ptr) {
            ...
            frames.push_back(confidence_frame.clone());
        }
...
namespace Utils {
    ...
    void convertDepthToDisplay(const cv::Mat& depth_frame, cv::Mat& display_frame) {
        if (depth_frame.empty()) {
            display_frame = cv::Mat::zeros(240, 180, CV_8UC3);
            return;
        }
        
        // Convert depth to 8-bit for display
        cv::Mat depth_8bit;
        depth_frame.convertTo(depth_8bit, CV_8U, 255.0 / 4000.0);
        
        // Apply colormap for better visualization
        cv::applyColorMap(depth_8bit, display_frame, cv::COLORMAP_JET);
    }
}